...
import {
  ...
  Loader,
  CheckCircle,
} from "lucide-react"; // import the icon components
...
export default function HomePage() {
  ...
  const SCRAPING_API_URL =
    "https://gnoo.onrender.com/api/v1/prediction/1b5ba2b2-40a4-4413-b75c-012609b5e7fb";
  const PDF_API_URL =
    "https://butterbot-ml2y.onrender.com/api/v1/prediction/88e6f717-db04-40bc-a3d5-753a7582b37d";
  ...
  const addData = async (event) => {
    ...
    if (newData.type === "URL" && !isValidURL(newData.value)) {
      setNewDataError(
        'Oops, invalid URL! Make sure your URL starts with "http" or "https", includes a domain and for best results, just copy it straight from your address bar.',
      );
      return;
    }
    ...
    if (response && response.status === 200) {
      ...
      try {
        ...
        // Fetch the bot data again to refresh the cards
        fetch("/api/getbot", {
          headers: {
            "Content-Type": "application/json",
          },
        ...
        {isAddDataModalOpen && (
          <div className="fixed z-10 inset-0 overflow-y-auto">
            <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center">
              <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:w-1/3 sm:max-w-lg sm:my-8 sm:align-middle sm:p-6">
                <div className="flex justify-between">
                  ...
                  <button onClick={() => setIsAddDataModalOpen(false)}>
                    <X className="h-6 w-6" />{" "}
                    {/* This is the X icon from lucide-react */}
                  </button>
                </div>
                <div className="mt-2">
                  <form onSubmit={addData} className="space-y-4">
                    ...
                    <Button type="submit" className={`w-full mt-4 ${loading ? 'bg-gray-700 hover:bg-gray-700' : ''}`}>
        ...
        {bots.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {bots.map((bot) => (
              <Card
                key={bot.id}
                className="w-full mb-4 shadow-md hover:shadow-lg transition-shadow duration-200 ease-in"
              >
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <Button
                        ...
                        size="icon"
                        onClick={() =>
                          router.push(`/customise?botName=${bot.botName}`)
                        }
                      >
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={() =>
                          router.push(`/conversation?botName=${bot.botName}`)
                        }
                      >
                ...
                <CardFooter className="flex justify-between">
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          ...
                          delete this bot and remove your data from our servers.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                  ...
                  <Button
                    className="text-xs"
                    onClick={() => {
                      setNewData({ botId: bot.id, type: "", value: "" });
